...
public class InvitationServices {
				...
				if (Untils.NotNull(foreign_id)) {
					...
					if (Untils.NotNull(fore.getIsHere() + "")) {
						if (fore.getIsHere() == 1) {// 在连
							...
							if (Untils.NotNull(fore.getFkInvitationId())
									&& fore.getFkBeuse() != 1) {
								flag = true;
							} else if (!Untils
									.NotNull(fore.getFkInvitationId())) {
								flag = true;
							} else {
								return fore.getName()
										+ " 目前存在处于使用状态的邀请函！无法关联新的邀请函。";
							}
						}
					}
					if (flag) {
				...
				if (Untils.NotNull(new_invitation.getFkAttachmentId())
					...
					if (Untils.NotNull(new_invitation.getFkAttachmentId())) {
						attch = attchServices.UpdataReturnObject(attachment,
								user, new_invitation.getFkAttachmentId(), 0);
					} else {
	...
	public String getsql(String foreign_id_q, String invitation_id_q,
			String is_use_q, String indate_q, String kind) {
